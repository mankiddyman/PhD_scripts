...
de_sf_base <- ne_states(country = "Germany", returnclass = "sf", scale = "large") |>
...
build_ribbon <- function() {
  ...
  ggplot(df, aes(x = year)) +
    ...
    scale_y_continuous(labels = label_number(scale = 1e-3, suffix = "k", prefix = "\u20ac")) +
    labs(
      title    = "**C** Q1\u2013avg\u2013Q3 salary band per state, 2019\u20132026",
      subtitle = "Shaded = Q1\u2013Q3 range \u00b7 Line = mean",
      x = NULL, y = "Gross monthly salary (\u20ac)"
    ) +
    base_theme +
    theme(
