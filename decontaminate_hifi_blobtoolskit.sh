...
#
#
micromamba activate blobtools2
#make a blobtools dir
mkdir -p blobtools_analysis_internal
cd blobtools_analysis_internal
...
#blobtools needs a csi format for its indexes
# Re-index with CSI format
samtools index -c ../old_hifi_mapped.bam
samtools index -c ../new_hifi_mapped.bam




# Process OLD assembly
echo ""
echo "=== Processing OLD HiFi assembly with BlobTools ==="
#taxid for D.binata is 4364
# Create BlobDir for old assembly
blobtools create \
  --fasta ../old_hifi_asm.fa \
  --taxid  4364 \
  --taxdump ./ \
  old_hifi_blobdir
...
blobtools add \
  --cov ../old_hifi_mapped.bam \
  --threads 48 \
  old_hifi_blobdir

# Add BLAST hits
blobtools add \
  --hits ../old_hifi_blast.out \
...
echo ""
echo "=== Processing NEW HiFi assembly with BlobTools ==="

# Create BlobDir for new assembly


blobtools create \
  --fasta ../new_hifi_asm.fa \
  --taxid  4364 \
  --taxdump ./ \
  new_hifi_blobdir
...
# Add coverage information
blobtools add \
  --cov ../new_hifi_mapped.bam \
  --threads 48 \
  new_hifi_blobdir

# Add BLAST hits
blobtools add \
  --hits ../new_hifi_blast.out \
  --taxrule bestsumorder \
  --taxdump ./ \
  new_hifi_blobdir


#mkay so we using singularity from here
cd /netscratch/dep_mercier/grp_marques/Aaryan/Drosera/binata/blobtools_analysis
singularity pull docker://genomehubs/blobtoolkit:4.4.5
mv blobtoolkit_4.4.5.sif btk.sif
# Rename for easier use
mv blobtoolkit_4.4.5.sif btk.sif
# Set your output directory
OUTDIR="/netscratch/dep_mercier/grp_marques/Aaryan/Drosera/binata/blobtools_analysis"
BLOBTOOLS_DIR="${OUTDIR}/blobtools_analysis_internal"
...
singularity exec \
  --bind ${PWD}:${PWD} \
  --env TMPDIR=${PWD}/local_tmp,TEMP=${PWD}/local_tmp,TMP=${PWD}/local_tmp \
  btk.sif \
  blobtools host --api-port 8021 --port 8081 ${PWD}/blobtools_analysis_internal


# below this is trash 

#!/bin/bash
# BlobToolKit Singularity setup for viewing existing data

# Now you can use it to view your blobdirs
# The container needs to bind your data directory

# Find your micromamba env path
CONDA_PREFIX=$(which geckodriver | xargs dirname | xargs dirname)
echo "Detected conda env: ${CONDA_PREFIX}"



# Find Firefox path (check common locations)
FIREFOX_PATH=""
if [ -f "${CONDA_PREFIX}/bin/firefox" ]; then
    FIREFOX_PATH="${CONDA_PREFIX}/bin"
elif [ -f "/usr/bin/firefox" ]; then
    FIREFOX_PATH="/usr/bin"
elif command -v firefox &> /dev/null; then
    FIREFOX_PATH=$(dirname $(which firefox))
fi

if [ -n "${FIREFOX_PATH}" ]; then
    echo "Detected Firefox at: ${FIREFOX_PATH}/firefox"
    FIREFOX_BIND="--bind ${FIREFOX_PATH}:${FIREFOX_PATH}"
else
    echo "Warning: Firefox not found, may need to install firefox in micromamba env"
    FIREFOX_BIND=""
fi

echo ""
echo "=== Starting interactive viewer for OLD assembly ==="


# test command
singularity shell \
  --bind ${BLOBTOOLS_DIR}:${BLOBTOOLS_DIR} \
  --bind ${CONDA_PREFIX}:${CONDA_PREFIX} \
  --bind ${FIREFOX_PATH}:${FIREFOX_PATH} \
  --env PATH=${CONDA_PREFIX}/bin:${FIREFOX_PATH}:$PATH \
  btk.sif 
...
singularity exec \
  ...
  ${FIREFOX_BIND} \
  --env PATH=${CONDA_PREFIX}/bin:${FIREFOX_PATH}:$PATH \
  btk.sif blobtools view \
  --host 0.0.0.0 \
  --port 8000 \
  ${BLOBTOOLS_DIR}/old_hifi_blobdir &

echo ""
echo "=== Starting interactive viewer for NEW assembly ==="
singularity exec \
  --bind ${BLOBTOOLS_DIR}:${BLOBTOOLS_DIR} \
  --bind ${CONDA_PREFIX}:${CONDA_PREFIX} \
  ${FIREFOX_BIND} \
  ...
  --host 0.0.0.0 \
  --port 8001 \
  ${BLOBTOOLS_DIR}/new_hifi_blobdir &
...
echo "To start the interactive viewer, run:"
echo ""
echo "singularity exec \\"
echo "  --bind ${BLOBTOOLS_DIR}:${BLOBTOOLS_DIR} \\"
echo "  --bind ${CONDA_PREFIX}:${CONDA_PREFIX} \\"
echo "  --env PATH=${CONDA_PREFIX}/bin:\$PATH \\"
echo "  btk.sif blobtools view \\"
echo "  --host 0.0.0.0 \\"
echo "  --port 8000 \\"
echo "  ${BLOBTOOLS_DIR}/old_hifi_blobdir"
echo ""
singulartiy exec \\
--bind ${BLOBTOOLS_DIR}:${BLOBTOOLS_DIR} \\
--bind ${CONDA_PREFIX}:${CONDA_PREFIX} \\
...
singularity exec --bind ${BLOBTOOLS_DIR}:${BLOBTOOLS_DIR} \
  btk.sif blobtools view \
  --format png \
  --view blob \
  --out ${BLOBTOOLS_DIR}/old_plots \
  ${BLOBTOOLS_DIR}/old_hifi_blobdir

echo "Or for the new assembly:"
echo ""
echo "singularity exec \\"
echo "  --bind ${BLOBTOOLS_DIR}:${BLOBTOOLS_DIR} \\"
echo "  --bind ${CONDA_PREFIX}:${CONDA_PREFIX} \\"
...
echo "  --host 0.0.0.0 \\"
echo "  --port 8001 \\"
echo "  ${BLOBTOOLS_DIR}/new_hifi_blobdir"
echo ""
echo "Then access via browser at: http://your-server-ip:8000"
echo ""
echo "Or to generate static plots without the viewer:"
echo ""
echo "singularity exec --bind ${BLOBTOOLS_DIR}:${BLOBTOOLS_DIR} \\"
echo "  btk.sif blobtools view \\"
echo "  --format png \\"
echo "  --view blob \\"
echo "  --out ${BLOBTOOLS_DIR}/old_plots \\"
echo "  ${BLOBTOOLS_DIR}/old_hifi_blobdir"
